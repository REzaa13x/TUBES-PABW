...
<div class="container px-6 mx-auto grid">
    <div class="flex justify-between items-center my-6">
        <h2 class="text-2xl font-semibold text-gray-700">Kelola Kampanye Relawan</h2>
                ...
                <tbody class="bg-white divide-y">
                    ...
                    <tr class="text-gray-700">
                        <td class="px-4 py-3">
                            <div class="flex items-center text-sm">
                                ...
                                <div>
                                    <p class="font-semibold">{{ $campaign->judul }}</p>
                                    <p class="text-xs text-gray-600">{{ $campaign->lokasi }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-sm">{{ $campaign->kategori }}</td>
                        <td class="px-4 py-3 text-xs">
                            <span class="px-2 py-1 font-semibold leading-tight rounded-full {{ $campaign->status == 'Aktif' ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100' }}">
                                {{ $campaign->status }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-2 text-sm">
                                ...
                                {{-- Tombol Delete --}}
                                <form action="{{ route('admin.campaigns.destroy', $campaign->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kampanye ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="4" class="px-4 py-6 text-center text-gray-500">
                            Belum ada kampanye. Silakan tambah baru.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
