...
<div class="container px-6 mx-auto">
    <h2 class="my-6 text-2xl font-semibold text-gray-700">Tambah Kontak Validator</h2>

    <div class="px-8 py-10 mb-8 bg-white rounded-3xl shadow-sm border border-gray-100 max-w-2xl">
        <form action="{{ route('admin.validator-contacts.store') }}" method="POST" class="space-y-6">
            @csrf
            
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Nama Lengkap</label>
                <input type="text" name="name" required class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:border-blue-500 focus:ring-0 text-sm" placeholder="Contoh: Budi Santoso">
            </div>

            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Nomor WhatsApp</label>
                <input type="text" name="phone" required class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:border-blue-500 focus:ring-0 text-sm" placeholder="Contoh: 628123456789">
