...
class DonationVerificationController extends Controller
{
    ...
    private function checkAdmin()
    {
        if (!Auth::user() || Auth::user()->role !== 'admin') {
            abort(403, 'Access denied. You must be an admin to access this endpoint.');
        }
    }
    ...
    /**
     * Delete proof of transfer for a donation transaction
     */
    public function deleteProof(Request $request, $order_id)
    {
        $this->checkAdmin();
        ...
        // Check if there's a proof of transfer to delete
        if (!$transaction->proof_of_transfer_path) {
            return response()->json([
                'success' => false,
                'message' => 'No proof of transfer found for this transaction'
            ], 404);
        }

        // Delete the proof file from storage
        if (\Storage::exists('public/' . $transaction->proof_of_transfer_path)) {
            \Storage::delete('public/' . $transaction->proof_of_transfer_path);
        }

        // Update the transaction to remove the proof path
        $transaction->update(['proof_of_transfer_path' => null]);

        return response()->json([
            'success' => true,
            'message' => 'Proof of transfer deleted successfully',
            'data' => [
                'order_id' => $transaction->order_id,
                'proof_deleted' => true
            ]
